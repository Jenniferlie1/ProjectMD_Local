...
class CreditScorePredictor:
    ...
    def __init__(self, model_path, meta_path):
        ...
        self.best_model_name = self.meta.get("best_model_name", "Best Model")
        self.train_columns = self.meta.get("train_columns", [])

    def build_input_df(self, inputs):
        ...
        for loan in LOAN_TYPES:
            row[loan] = 1 if loan in inputs["loan_types"] else 0

        df = pd.DataFrame([row])

        if self.train_columns:
            df = df.reindex(columns=self.train_columns)

        return df

    def predict(self, inputs):

        input_df = self.build_input_df(inputs)

        label_idx = self.pipeline.predict(input_df)[0]
...
st.caption(f"Model: {predictor.best_model_name}  |  Classes: Poor, Standard, Good")
...
if submitted:
    ...
    inputs = dict(
        ...
        delay_from_due=delay_from_due,
        num_delayed_payments=num_delayed_payments,
        payment_of_min=payment_of_min,
        ...
        loan_types=selected_loans,
    )

    label, proba = predictor.predict(inputs)
    ...
    st.session_state.history.append({
        **inputs,
        "Prediction": label,
        "Loan Types": ", ".join(selected_loans),
    })
    ...
    with col2:
        if proba is not None:
            ...
            prob_df = pd.DataFrame({"Probability (%)": [p * 100 for p in proba]}, index=labels,)
